...
expect.extend({
  toMatchSVGSnapshot(
    ...
    if (!root) {
      return fail('A falsy Component was passed to toMatchSVGSnapshot')
    }
    if (!root.props) {
      return fail('A Component without props was passed to toMatchSVGSnapshot')
    }
    if (!root.type) {
      return fail('A Component without a type was passed to toMatchSVGSnapshot')
    }
